     ...
     <form action="/dashboard/transaction/{{ $transaction->id }}" method="post" class="row g-3 col-lg-8">
          ...
          <div class="col-md-6">
               ...
               <select class="form-select @error('kode_member') is-invalid @enderror" name="kode_member" id="kode_member" required>
                    @foreach ($user as $us)
                         @if(old('kode_member', $transaction->kode_member) == $us->kode_member )
                              <option value="{{ $us->kode_member }}" selected >{{ $us->kode_member }} - {{ $us->nama }}</option>
                         @else
                              <option value="{{ $us->kode_member }}">{{ $us->kode_member }} - {{ $us->nama }}</option>
                         @endif
                    @endforeach
               </select>
               @error('kode_member')
               ...
               <select class="form-select @error('book_id') is-invalid @enderror" name="book_id" id="book_id" required>
                    @foreach ($book as $bk)
                         @if(old('book_id', $transaction->book_id) == $bk->id )
                              <option value="{{ $bk->id }}" selected >{{ $bk->kode_buku }} - {{ $bk->nama }}</option>
                         @else
                              <option value="{{ $bk->id }}">{{ $bk->kode_buku }} - {{ $bk->nama }}</option>
                         @endif
                    @endforeach
               </select>
               @error('book_id')
