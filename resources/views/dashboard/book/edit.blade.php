...
@section('content')
     ...
     <form action="/dashboard/book/{{ $book->id }}" method="post" class="row g-3 col-lg-8">
          @csrf @method('put')
          <div class="col-md-3">
               <label for="kode_buku" class="form-label">Kode_buku</label>
               <input type="text" class="form-control @error('kode_buku') is-invalid @enderror" name="kode_buku" id="kode_buku" value="{{ old('kode_buku', $book->kode_buku) }}" required>
               @error('kode_buku')
